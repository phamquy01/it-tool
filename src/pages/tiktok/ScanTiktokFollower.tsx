...
const ScanTiktokFollower = () => {
  ...
  const fetchData = async (cursor: string = '', secUid: string = '') => {
    const res = await http.post<ScanFollowingTiktokResponse>(
      'https://api.autozalo.com/api/scan_tiktok_follower',
      {
        user: user.trim(),
        cursor: cursor.trim(),
        secUid: secUid.trim(),
  ...
  const handleScan = async () => {
    ...
    try {
      ...
      if (allGroups.length >= targetLimit) {
        ...
        showToast('scan_tiktok_follower.fetch_success', 'success');
        return;
      }
      ...
      trackUsage('Scan tiktok follower');
      showToast('scan_tiktok_follower.scan_success', 'success');
    } catch (err) {
  ...
  const handleExportExcel = () => {
    ...
    try {
      const excelData = dataScanTiktokFollowing.map((group, index) => ({
        STT: index + 1,
        [t('scan_tiktok_follower.user_id')]: group.id,
        [t('scan_tiktok_follower.user_nickname')]: group.nickname,
        [t('scan_tiktok_follower.user_username')]: group.uniqueId,
        [t('scan_tiktok_follower.user_video_count')]: group.videoCount,
        [t('scan_tiktok_follower.follower_count')]: group.followerCount,
        [t('scan_tiktok_follower.following_count')]: group.followingCount,
        [t('scan_tiktok_follower.friend_count')]: group.friendCount,
        [t('scan_tiktok_follower.heart_count')]: group.heartCount,
      }));
      ...
      const timestamp = new Date()
        ...
        .replace(/\//g, '-');
      const fileName = `Tiktok_follower_${timestamp}.xlsx`;

      XLSX.writeFile(workbook, fileName);

      showToast('scan_tiktok_follower.export_success', 'success');
    } catch (error) {
  ...
  useEffect(() => {
    const saved = localStorage.getItem('scanTiktokFollowerState');
    if (!saved) return;
    const parsed = saved ? JSON.parse(saved) : {};
    if (Date.now() - parsed.timestamp > EXPIRE_TIME) {
      localStorage.removeItem('scanTiktokFollowerState');
      return;
    }
            ...
            <div className="flex items-center gap-2 w-full">
              ...
              >
                ...
                <span className="truncate">
                  {loading ? (
                    <>{t('scan_tiktok_follower.scanning')}</>
                  ) : (
                    <>{t('scan_tiktok_follower.button_scan')}</>
                  )}
                </span>
              </button>
              <button
              ...
              >
                ...
                <span className="truncate">
                  {t('scan_tiktok_follower.button_export')}
                </span>
              </button>
            </div>
                  ...
                  <div className="space-y-2 min-w-0">
                    ...
                    <div>
                      <span className="text-xs font-medium text-slate-500 dark:text-slate-400 block mb-1">
                        {t('scan_tiktok_follower.user_id')}
                      </span>
                    ...
                    <div className="flex flex-col sm:flex-row gap-2">
                      <div className="flex-1">
                        <span className="text-xs font-medium text-slate-500 dark:text-slate-400 block mb-1">
                          {t('scan_tiktok_follower.user_nickname')}
                        </span>
                        <span className="text-xs font-semibold text-slate-700 dark:text-slate-200 break-words">
                          {follow.nickname}
                        </span>
                      ...
                      <div className="flex-1">
                        <span className="text-xs font-medium text-slate-500 dark:text-slate-400 block mb-1">
                          {t('scan_tiktok_follower.user_username')}
                        </span>
                        <span className="text-xs font-semibold text-slate-700 dark:text-slate-200 break-words">
                          {follow.uniqueId}
                        </span>
                    ...
                    <div className="flex flex-col sm:flex-row gap-2">
                      <div className="flex-1">
                        <span className="text-xs font-medium text-slate-500 dark:text-slate-400 block mb-1">
                          {t('scan_tiktok_follower.user_video_count')}
                        </span>
                        <span className="text-xs font-semibold text-slate-700 dark:text-slate-200 break-words">
                          {follow.videoCount}
                        </span>
                      ...
                      <div className="flex-1">
                        <span className="text-xs font-semibold text-slate-500 dark:text-slate-400 block mb-1">
                          {t('scan_tiktok_follower.follower_count')}
                        </span>
                        <span
                    ...
                    <div className="flex flex-col sm:flex-row gap-2">
                      <div className="flex-1">
                        <span className="text-xs font-semibold text-slate-500 dark:text-slate-400 block mb-1">
                          {t('scan_tiktok_follower.following_count')}
                        </span>
                      ...
                      <div className="flex-1">
                        <span className="text-xs font-semibold text-slate-500 dark:text-slate-400 block mb-1">
                          {t('scan_tiktok_follower.friend_count')}
                        </span>
                        <span className="inline-block py-0.5 rounded-full text-xs font-semibold">
                          {follow.friendCount}
                        </span>
                    ...
                    <div>
                      <span className="text-xs font-semibold text-slate-500 dark:text-slate-400 block mb-1">
                        {t('scan_tiktok_follower.heart_count')}
                      </span>
                      <span
                        className={`inline-block py-0.5 rounded-full text-xs font-semibold`}
                      >
...
export default ScanTiktokFollower;
