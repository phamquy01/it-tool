...
const ScanPostByKeyword = () => {
  ...
  const [dataScanPostBykeyword, setDataScanPostBykeyword] = useState<
    PostKeywordData[] | null
  >(null);
  ...
  const handleScan = async () => {
    ...
    try {
      ...
      if (allGroups.length >= targetLimit) {
        ...
        showToast('scan_post_by_keyword.fetch_success', 'success');
        return;
      }

      if (allGroups.length > 0) {
        setDataScanPostBykeyword([...allGroups]);
      }
      ...
      showToast('scan_post_by_keyword.scan_success', 'success');
    } catch (err) {
  ...
  const handleExportExcel = () => {
    if (!dataScanPostBykeyword || dataScanPostBykeyword.length === 0) {
      showToast('Không có dữ liệu để xuất', 'error');
      return;
    }

    try {
      const excelData = dataScanPostBykeyword.map((post, index) => ({
        STT: index + 1,
        [t('scan_post_by_keyword.content_post')]: post.content,
        [t('scan_post_by_keyword.link_post')]: post.url,
        [t('scan_post_by_keyword.id_post')]: post.id,
        [t('scan_post_by_keyword.creation_time_post')]: post.creationTime,
        [t('scan_post_by_keyword.image_post')]: post.image,
      }));
      ...
      const timestamp = new Date()
        ...
        .replace(/\//g, '-');
      const fileName = `Posts_${keyword}_${timestamp}.xlsx`;

      XLSX.writeFile(workbook, fileName);

      showToast('scan_post_by_keyword.export_success', 'success');
    } catch (error) {
        ...
        <div className="space-y-3 sm:space-y-4 min-w-0">
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-3 sm:gap-4 min-w-0">
            <div className="lg:col-span-2 min-w-0">
              <div className="flex justify-between items-start min-w-0">
                ...
                >
                  {t('scan_post_by_keyword.cookie_guide')}
                </label>
              </div>
              <div
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-3 sm:gap-4 min-w-0">
            <div className="lg:col-span-2 min-w-0">
              ...
              >
                {t('scan_post_by_keyword.keyword')}
              </label>
              <div
                className={`flex items-center gap-2 px-3 bg-white dark:bg-zinc-900 border border-zinc-300 rounded w-full min-w-0 transition-opacity duration-200 ${
                  loading ? 'opacity-50 cursor-not-allowed' : ''
            ...
            <div className="space-y-2 min-w-0">
              ...
              <div className="w-full bg-slate-200 dark:bg-zinc-700 rounded-full h-1.5 min-w-0">
                <div
                  ...
                  style={{
                    width: `${Math.min(
                      ((dataScanPostBykeyword?.length || 0) / Number(limit)) *
                        100,
                      100
                    )}%`,
                  }}
        ...
        <div className="mt-4 sm:mt-6 min-w-0 overflow-hidden">
          ...
          <div className="block lg:hidden space-y-3 min-w-0">
            {!dataScanPostBykeyword || dataScanPostBykeyword.length === 0 ? (
              ...
              dataScanPostBykeyword.map((post, index) => (
                <div
                  key={post.id}
                  ...
                  <div className="space-y-2 min-w-0">
                    ...
                    <div className="min-w-0">
                      <span className="text-xs font-medium text-slate-500 dark:text-slate-400 block mb-1">
                        {t('scan_post_by_keyword.content_post')}
                      </span>
                    ...
                    <div className="min-w-0">
                      <span className="text-xs font-medium text-slate-500 dark:text-slate-400 block mb-1">
                        {t('scan_post_by_keyword.link_post')}
                      </span>
                      <a
                        href={post.url}
                        target="_blank"
                        rel="noopener noreferrer"
                        className="text-xs text-blue-600 dark:text-blue-400 hover:underline break-all word-break leading-relaxed line-clamp-1"
                      >
                        {post.url}
                      </a>
                    </div>
                    <div className="min-w-0">
                      <span className="text-xs font-medium text-slate-500 dark:text-slate-400 block mb-1">
                        {t('scan_post_by_keyword.id_post')}
                      </span>
                      <span className="text-xs text-slate-700 dark:text-slate-200 break-all word-break">
                        {post.id}
                      </span>
                    </div>
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 min-w-0">
                      <div className="min-w-0">
                        <span className="text-xs font-medium text-slate-500 dark:text-slate-400 block mb-1">
                          {t('scan_post_by_keyword.creation_time_post')}
                        </span>
                    ...
                    {post.image && (
                      <div className="min-w-0">
                        <span className="text-xs font-medium text-slate-500 dark:text-slate-400 block mb-1">
                          {t('scan_post_by_keyword.image_post')}
                        </span>
                        <div className="text-xs text-slate-700 dark:text-slate-200 break-all word-break leading-relaxed">
                          {post.image.includes('|') ? (
                              ...
                              {post.image.split('|').map((imageUrl, index) => (
                                <div key={index} className="relative group">
                                  <img
                                    src={imageUrl.trim()}
                                    alt={`${t(
                                      'scan_post_by_keyword.image_post'
                                    )} ${index + 1}`}
                                    className="w-12 h-12 object-cover rounded border border-slate-200 dark:border-zinc-600 shadow-sm hover:shadow-md transition-shadow duration-200 cursor-pointer"
                                    onClick={() => {
                                  ...
                                  />
                                  <div className="hidden w-12 h-12 bg-slate-100 dark:bg-zinc-700 rounded border border-slate-200 dark:border-zinc-600 items-center justify-center text-xs text-slate-500 dark:text-slate-400">
                                    Ảnh {index + 1}
                                  </div>
                                </div>
                              ))}
                            ...
                            <div className="relative group max-w-fit">
                              <img
                                src={post.image}
                                alt={t('scan_post_by_keyword.image_post')}
                                className="w-16 h-16 object-cover rounded border border-slate-200 dark:border-zinc-600 shadow-sm hover:shadow-md transition-shadow duration-200 cursor-pointer"
                                onClick={() => {
                              ...
                              />
                              <div className="hidden w-16 h-16 bg-slate-100 dark:bg-zinc-700 rounded border border-slate-200 dark:border-zinc-600 items-center justify-center text-xs text-slate-500 dark:text-slate-400">
                                {t('scan_post_by_keyword.view_image')}
                              </div>
                            </div>
                          )}
                        </div>
              ...
              <table className="w-full text-xs">
                <thead className="sticky top-0 z-10 bg-slate-100 dark:bg-zinc-900">
                  <tr>
                    ...
                    <th className="px-2 py-2 font-semibold text-slate-700 dark:text-zinc-200 text-left whitespace-nowrap w-80">
                      {t('scan_post_by_keyword.link_post')}
                    </th>
                    ...
                    <th className="px-1 py-2 font-semibold text-slate-700 dark:text-zinc-200 text-center whitespace-nowrap w-20">
                      {t('scan_post_by_keyword.creation_time_post')}
                    </th>
                  ...
                  dataScanPostBykeyword.length === 0 ? (
                    ...
                    dataScanPostBykeyword.map((post, index) => (
                      ...
                      >
                        ...
                        <td className="px-2 py-2 text-blue-600 dark:text-blue-400 text-left max-w-[300px]">
                          <a
                            href={post.url}
                            target="_blank"
                            rel="noopener noreferrer"
                        ...
                        <td
                          className="px-1 py-2 text-slate-700 dark:text-slate-200 text-center whitespace-nowrap w-24"
                          onClick={() => {
                        ...
                        <td
                          className="px-2 py-2 text-slate-700 dark:text-slate-200 text-center font-semibold max-w-[300px]"
                          onClick={() => {
                        ...
                        <td className="px-1 py-2 text-emerald-700 dark:text-emerald-400 text-center whitespace-nowrap w-20 font-medium">
                          {formatDate(post.creationTime)}
                        </td>

                        <td className="px-2 py-2 text-slate-700 dark:text-slate-200 text-left w-30">
                          {post.image ? (
                            post.image && (
                              <div className="flex flex-wrap gap-1 justify-start">
                                {post.image.split('|').map((img, imgIndex) => (
                                  <img
                                    key={imgIndex}
                                    src={img.trim()}
                                    alt={`Ảnh ${imgIndex + 1}`}
...
export default ScanPostByKeyword;
